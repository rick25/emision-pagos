...
const FilaCampoEpagos = ({ dato, opcionesEpagos, eliminarCampo, asignarOpcionACampo }) => {
  const [opcionesSelect, setOpcionesSelect] = useState([])

  useEffect(() => {
    const opciones = opcionesEpagos.map(({camID, pcaDescripcion}) => <Select.Option key={camID} value={camID}>{pcaDescripcion}</Select.Option>)
  ...
  //   setCorrespondencia(camID, value)
  // }
  const { camDescripcion, camNombre, camObligatorio, camTipo, camID, pcaID } = dato

  const INTEGER = <InputNumber
                    style={{ width: '100%' }}
                    min={1}
                    // onChange={e => changeOption(e, camID)}
                  >
                  </InputNumber>
  const VARCHAR = <Select
                    style={{ width: '100%' }}
                    name={camID}
                    placeholder="Elija un valor"
                    // onChange={e => changeOptions(e, camID)}
                    // onChange={(e) => handleChangeSelect(e, pcaID)}
                    onChange={(e) => asignarOpcionACampo(pcaID, e)}
                    rules={[{ required: true, message: 'Complete este campo para continuar'}]}
                  >
                    {opcionesSelect}
                  </Select>
  const BOOLEAN = <Radio.Group
  ...
  const tiposDeCampos = { INTEGER, VARCHAR, BOOLEAN }
